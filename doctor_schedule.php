...
$update_stmt = $conn->prepare("UPDATE schedules SET status = 'expired' WHERE date = ? AND end_time < ? AND status = 'active'");
$update_stmt->bind_param("ss", $today, $current_time);
$update_stmt->execute();

// Fetch active schedules for logged-in doctor
$select_stmt = $conn->prepare("SELECT * FROM schedules WHERE doctor_id = ? AND status = 'active' ORDER BY date");
$select_stmt->bind_param("i", $doctor_id);
$select_stmt->execute();
$schedules = $select_stmt->get_result();
?>
<!DOCTYPE html>
<html lang="en">
<head>
